 ...
 */
export class Input extends FrakClass {
  init(engine, canvas) {
  ...
  /**
	 ...
	 */
  bind(key, callback, obj) {
    if (key && callback && obj) {
      if (key in this.keymap) this.bindings[this.keymap[key]] = [obj, callback];
      else this.bindings[key.charCodeAt(0)] = [obj, callback];
    }
  }
  ...
  sendEvent(funcName) {
    ...
    activated.sort((a, b) => b.getPriority(funcName) - a.getPriority(funcName));
  ...
  onTap(event) {
    if (event) {
      const newPos = this.translateCoordinates(this.position, event.center.x, event.center.y);
  ...
  onPinch(event) {
    ...
    if (event) {
      ...
      const scale = event.scale - this.lastPinch;
  ...
  onRotate(event) {
    if (event) {
      ...
      const rotation = event.rotation - this.lastRotation;
  ...
  onMouseWheelMOZ(event) {
    if (!event) return;

    event.preventDefault();

    const direction = event.detail > 0 ? 1 : event.detail < 0 ? -1 : 0;
  ...
  processKeyEvents() {
    for (const key in this.keyStates) {
      if (this.bindings[key]) {
        const method = this.bindings[key];
