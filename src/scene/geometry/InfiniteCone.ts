...
class InfiniteCone {
	...
	containsPoint(point): any {
		return this.ray.distanceOfPoint(point)<this.radiusAtPoint(point);
	}
	...
	containsSphere(boundingSphere): any {
		return this.ray.distanceOfPoint(boundingSphere.position)<this.radiusAtPoint(boundingSphere.position)+boundingSphere.radius;
	}
