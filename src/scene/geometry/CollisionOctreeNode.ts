...
class CollisionOctreeNode {
	...
	constructor(center, size, parent) {
		this.parent=false;
		this.depth=0;
		this.subnodes=false;
		this.bounds=false;
	...
	// addFace(i0, i1, i2, meshIndex): any {
	// 	if (!this.faces)
	// 		this.faces={};
	...
	rayIntersectGeometry(worldRay, collideInvisible): any {
		if (!this.hasGeometry()) {
			return false;
		}
	...
	getNodesWithGeometry(ray, list): any {
	...
	/** Returns the ray intersection result nearest to the ray's origin.
		@param ray Instance of {Ray}
		*/
	getNearestRayCollision(localRay, worldRay, collideInvisible): any {
		var nodes=[];
