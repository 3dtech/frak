...
type WebGLIndexFormat = WebGL2RenderingContext['UNSIGNED_INT'] | WebGL2RenderingContext['UNSIGNED_SHORT'];
...
class Submesh {
	...
	boundingSphere: any;
	edges: any;
	indexType: WebGLIndexFormat = WebGL2RenderingContext.UNSIGNED_SHORT;
	...
	_calculateTangents4D(): any {
		...
		for (let i = 0; i < this.tangents4D.length; i += 4) {
	...
	calculateTangents(): any {
		let tan1 = new Float32Array(this.positions.length);
		let tan2 = new Float32Array(this.positions.length);

		let uv = this.texCoords2D[0];
		let v1 = vec3.create();
		let v2 = vec3.create();
		let v3 = vec3.create();
		let w1 = vec2.create();
		...
		for (var i = 0; i < this.faces.length; i += 3) {
			let i1 = this.faces[i];
			let i2 = this.faces[i + 1];
			let i3 = this.faces[i + 2];
		...
		for (var i = 0; i < this.normals.length; i += 3) {
			...
			vec3.cross(tmp, n, t);
			let w = vec3.dot(tmp, tan) < 0.0 ? -1.0 : 1.0;
	...
	calculateBarycentric(): any {
		...
		for (let i = 0; i < this.faces.length; i += 3) {
			let idx0 = this.faces[i];
			let idx1 = this.faces[i + 1];
			let idx2 = this.faces[i + 2];
	...
	recalculateBounds(): any {
		...
		for (let i = 0, l = this.positions.length; i < l; i += 3) {
