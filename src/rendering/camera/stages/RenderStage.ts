...
class RenderStage {
	...
	start(context, engine, camera) {
		...
		for (let i = 0; i < this.substages.length; i++) { this.substages[i].start(context, engine, camera); }
	}
	...
	/** Restores the rendering context for this render stage and its substages. */
	restoreContext(context: RenderingContext) {
		this.onContextRestored(context);

		for (const substage of this.substages) {
			substage.restoreContext(context);
		}
	}
	...
	/** Called when the rendering context is restored (for example after a context lost event) */
