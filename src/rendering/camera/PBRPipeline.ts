...
class PBRPipeline extends PostProcessRenderStage {
	...
	initDebugger(context) {
		...
		this.debugger.quads.push({ quad: createQuad(x += size, y, size, size), texture: this.generator.oitTargets.targets[0] });
		this.debugger.quads.push({ quad: createQuad(x += size, y, size, size), texture: this.generator.oitTargets.targets[1] });
	}
}
