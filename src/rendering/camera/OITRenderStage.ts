...
class OITRenderStage extends RenderStage {
	diffuseFallback: any;
	oitClearColor: any;
	transparencyTarget: any;
	transparencySampler: any;
	transparencyWeight: any;
	transparencyWeightSampler: any;
	...
	renderTransparentBatches(context, scene, camera, material): any {
	...
	renderAlphaMapped(context, scene, camera): any {
		var gl = context.gl;
		gl.depthMask(true);
	...
	renderOpaque(context, scene, camera): any {
		var gl = context.gl;
		gl.enable(gl.DEPTH_TEST);
		gl.depthFunc(gl.LESS);
	...
	renderPass(context, scene, camera, renderColor) {
		var gl = context.gl;
