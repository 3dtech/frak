...
import type DefinitionsHelper from "./DefinitionsHelper";

declare global {
	var WebGLDebugUtils: any,
		fallbackTexture: any,
		fallbackCubeTexture: any;
}
...
class RenderingContext {
	...
	*/
	...
	restore() {
		...
		let contextLost = this.gl.getError(); // Remove the GL_CONTEXT_LOST_WEBGL error
		let ctx = this;
		let engine = this.engine;

		// Restore shaders
		let texturesManager = engine.assetsManager.texturesManager;
		for (let i in texturesManager.cache) {
			let texture = texturesManager.cache[i];

			texture.onContextRestored(ctx);
		}

		// Restore geometry
		let rootNode = engine.scene.root;

		rootNode.onEachChildComponent(function (component) {
			if (component instanceof RendererComponent ||
				component instanceof TextComponent) { component.onContextRestored(ctx); }
		});

		// Restore lights
		for (let j = 0; j < engine.scene.lights.length; ++j) {
...
export type { Canvas };
export { RenderingContext as default };
